...
python labopDistribution.py --l 67 --m 50 --r 15 --u 1 --out outputfolder --seed 200 --num-sets 50
    ...
    --u = number of MUST-SELECT per guard
...
Each guard will have exactly `r` CANNOT-SELECT entries and exactly `u` MUST-SELECT entries (chosen from the remaining slots). All other slots are OK.
...
def make_guard_row(guard_idx: int, l: int, r: int, u: int, seed: int | None = None) -> Tuple[List[str], str, str, str, str, str]:
    """Create a single guard CSV row.

    Returns: (cells_for_slots, id, start time, completion time, email, name)
    ...
    if u > len(remaining):
        raise ValueError(f"u ({u}) is larger than available slots after rejects: {len(remaining)}")
    must = set(random.sample(remaining, u)) if u > 0 else set()
    cells = []
    for i in range(l):
        if i in must:
...
def generate_dataset(l: int = 67, m: int = 50, r: int = 10, u: int = 3, k_min: int = 1, k_max: int = 3, s: int = 3, out_dir: str = "outputs", seed: int | None = 42) -> str:
    """Generate dataset and return path to output directory.

    This creates a folder under out_dir named dataset_r{r}_u{u}_m{m}_l{l}_{seed}.
    """
    if u + r > l:
        raise ValueError("u + r cannot exceed total slots l")

    random.seed(seed)
    slots = gen_slot_times(l)
    guard_rows = []
    for gi in range(m):
        cells, gid, st, ct, email, lname, fname = make_guard_row(gi, l, r, u, seed)
    ...
    folder = os.path.join(out_dir, f"dataset_r{r}_u{u}_m{m}_l{l}")
...
def parse_args():
    ...
    p.add_argument("--u", type=int, default=1, help="number of MUST-SELECT per guard")
...
if __name__ == "__main__":
    ...
    for i in range(args.num_sets):
        ...
        folder = generate_dataset(l=args.l, m=args.m, r=args.r, u=args.u, out_dir=args.out, seed=s)
